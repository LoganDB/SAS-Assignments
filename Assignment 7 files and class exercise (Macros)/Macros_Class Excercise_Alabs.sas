...
DATA models;
INFILE 'C:\Users\Sumeet Bansal\Google Drive\alabs\Analytics\SAS\Assignments\Assignment 6 files (Macros)\models.txt';
...
PROC IMPORT OUT= WORK.client 
            DATAFILE= "C:\Users\Sumeet Bansal\Google Drive\alabs\Analytics\SAS\Assignments\Assignment 6 files (Macros)\client.txt" 
            DBMS=TAB REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
PROC PRINT;
...
retain cum 0;
cum = cum + product;
if eof = 1 then call symput('mac_cum',cum);run;
proc print;
run;
%put &mac_cum;
...
PROC IMPORT OUT= WORK.supplier 
            DATAFILE= "C:\Users\Sumeet Bansal\Google Drive\alabs\Analytics\SAS\Assignments\Assignment 6 files (Macros)\supplier.txt" 
            DBMS=TAB REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
PROC PRINT;
